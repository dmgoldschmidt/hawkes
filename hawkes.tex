...
There is one important difference between the base process and all the child processes.  Namely, the base process
...
$\lambda_{i0} := \lambda_0$ for all $i$, $t_{ij} := t_i-t_j$ for $i > 0$. 

\subsection{The Model}
We want to explain an observed arrival stream $\{(t_0,\mu(0),\dots,(t_N,\mu(N)\}$ as the output
of a Hawkes process.  All arrival times are given as elapsed times from $t_0$, the arrival time of the first
base process event, so $t_{i0}$ is just $t_i$.
Thus, if $t_i$ is the arrival time of an event generated by process $\P_j$ for some index $j < i$, and
$\P_j$ has generated $k_{ij}-1$ previous events prior to time $t_i$, then 
...
\k_{ij} =
\begin{cases}
  \frac{\sigma_{m}}{\rho_{m}}(1-e^{-\rho_{m}t_{ij}})&\text{for $m = \mu(j)$ and $j > 0$,} \\
  \lambda_0t_i & \text{for $j = 0$}.
\end{cases}
$$
So \eqref{p_ij} becomes
\begin{equation}\label{q_ij}
  q_{ij} := \frac{\lambda_{ij}^{\k_{ij}}t_{ij}^{(\k_{ij}-1)}}{\Gamma(\k_{ij})}
    e^{-\lambda_{ij}t_{ij}}\quad (j < i),
\end{equation}
where we have replaced $k_{ij}$ in \eqref{p_ij} by $\k_{ij}$ and $(k_{ij}-1)!$ by $\Gamma(\hat{k_{ij}})$
(because $\k_{ij}$ is not in general an integer).

Then our likelihood function becomes a product of mixture distributions, as discussed in section \eqref{MD}:
$$
L = \prod_{i=0}^NPd(T_i = t_i)
$$
where 
...
At first glance, it appears that we might have a large number of states because the number of states is
equal to the number of observations,  However, note first that $q_{ij} = 0$ for $j \ge i$, and second that
$q_{ij}$ decays exponentially in $t_i-t_jq$, so in practice we can ignore states $j << i$.
...
\tilde{Q}(\theta) = \sum_{i=0}^N\sum_{j=0}^{i-1}\omega_{ij}'\log(\omega_jq_{ij}),
\end{equation}
...
\begin{align*}
  0 &= \frac{\partial}{\partial\omega_j}\left(\tilde{Q} + \eta(\sum_{j=0}^N\omega_j - 1)\right) \\
  &= \frac{1}\omega_j\sum_{i=0}^N\omega_{ij}' - \eta.
\end{align*}
We conclude that
\begin{equation}\label{omega_j:0}
  \begin{split}
  \eta\omega_j &= \sum_{i=0}^N\omega_{ij}',\quad\text{whence} \\
  \eta\sum_{j=0}^N\omega_j &= \sum_{i=0}^N\sum_{j=0}^N\omega'_{ij}.
  \end{split}
\end{equation}
Since
$$
...
\begin{equation}\label{omega_j}
  \omega_j = \frac{1}{N}\sum_{i=0}^N\omega'_{ij}.
\end{equation}

%Next: check equations for \k_{ij} = \frac{1}{\rho_m}(\sigma_m - \lambda_{ij})
%Then fix eqref{Q1}. It's now wrong.
%Then fix eqref{lambda0}
To solve for the remaining parameters, we first expand \eqref{Q}:
\begin{equation}\label{Q1}
  \begin{split}
    \tilde{Q} &= \sum_{i=0}^N\sum_{j=0}^{i-1}\omega'_{ij}\log\left(\omega_j\frac{\lambda_{ij}^{\k_{ij}}t_{ij}^{(\k_{ij}-1)}}
          {\Gamma(\k_{ij})}e^{-\lambda_{ij}t_{ij}} \right)\\
    &= \sum_{i=0}^N\sum_{j=0}^{i-1}\omega'_{ij}\left(\log(\omega_j)+\k_{ij}\log(\lambda_{ij})+(\k_{ij}-1)\log(t_{ij})
  -\log(\Gamma(\k_{ij})) -\lambda_{ij}t_{ij}\right)\\
  \end{split}
\end{equation}
...
\begin{equation}\label{lambda_0}
  \begin{split}
  0 &= \frac{\partial\tilde{Q}}{\lambda_0} = \frac{\partial}{\partial\lambda_0}\sum_{i=0}^N\omega'_{i0}\left(\lambda_0t_0\log(\lambda_0t_0)+(\lambda_0t_0-1)
  \log(t_0)-\log(\Gamma(\lambda_0t_0) - \lambda_ot_0\right)\\
  &= \sum_{i=0}^N\omega'_{i0}\left(t_0(\log(\lambda_0t_0)+1)+ t_0\log(t_0)-t_0\frac{\Gamma'(\lambda_0t_0)}{\Gamma(\lambda_0t_0)}-t_0\right)\\
  &=  \sum_{i=0}^N\omega'_{i0}\left( t_0(\log(\lambda_0) +2t_0\log(t_0)  -\psi(\lambda_0t_0)\right).
  \end{split}
\end{equation}
...
Since $\tilde{Q}$ depends on $\sigma_m$ and $\rho_m$ only through $\lambda_{ij}$ and $\k_{ij}$,
we first compute, for $j\in J(m)$ and all $i$,
\begin{equation}\label{d_dsigma,rho}
  \begin{split}
    \frac{\partial\lambda_{ij}}{\partial\sigma_m} &= e^{-\rho_mt_{ij}}, \\
    \frac{\partial\lambda_{ij}}{\partial\rho_m} &= -t_{ij}\lambda_{ij}, \\
    \frac{\partial\k_{ij}}{\partial\sigma_m} &= \frac{1}{\sigma_m}\k_{ij},\\
    \frac{\partial\k_{ij}}{\partial\rho_m} &= t_{ij}\frac{\sigma_m}{\rho_m}e^{-\rho_mt_{ij}}-\frac{1}{\rho_m}\k_{ij}.
  \end{split}
\end{equation}
...
\begin{equation}\label{dQ_dsigma,rho}
  0 = \frac{\partial\tilde{Q}}{\partial{x}} = \sum_{i=0}^N\sum_{j\in J(m)}\frac{\partial\tilde{Q}}{\partial\lambda_{ij}}\frac{\partial\lambda_{ij}}{\partial{x}} +
...
\frac{\partial}{\partial\lambda_0}\sum_{i=0}^N\omega'_{i0}\log\left(\omega_0\frac{\lambda_0^{\lambda_0t_0}t_0^{(\lambda_0t_0-1)}}{\Gamma(\lambda_0t_0)}e^{-\lambda_0t_0} \right)\\
  &= \frac{\partial}{\partial\lambda_0}
  \sum_{i=0}^N\omega'_{i0}\left(\log(\omega_0)+\lambda_0t_0\log(\lambda_0)+(\lambda_0t_0-1)\log(t_0)
