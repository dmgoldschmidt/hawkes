...
\end{equation}
nd we also see that
\begin{equation}\label{dk_lambda,sigma}
  \begin{split}
...
Now for $j = 0$,  \eqref{dQ_dk} and \eqref{dk_lambda,sigma} immediately imply
...
$$
then using \eqref{dk_lambda,sigma} we get
...
\begin{equation}\label{dk_hat_drho}
  \begin{split}
    ...
    &= \frac{1}{\rho}(\sigma'(\rho)-\k_{ij} + \sigma(\rho)t_{ij}e^{-\rho t_{ij}}).
  \end{split}
\end{equation}
...
&= -\sum_{i>j>0}\frac{\omega'_{ij}}{2\k_{ij}}\frac{1}{\rho}(\sigma'(\rho)-\k_{ij} + \sigma(\rho)t_{ij}e^{-\rho t_{ij}}).
\end{split}
\end{equation}
...
\end{document}
    ...
    \frac{\partial{S}}{\partial\rho} &= \frac{\sigma}{\rho^2}\sum_{j=1}^{N-1}\left(1-e^{-\rho t_{Nj}} - \rho t_{Nj}e^{-\rho t_{Nj}}\right).
Moreover, \eqref{dk_drho(0)} implies that
\begin{equation}\label{dS_drho(0)}
  \left.\frac{\partial S}{\partial\rho}\right|_{\rho=0} = -\sum_{j=1}^{N-1}\left.\frac{\partial\k_{Nj}}{\partial\rho}\right|_{\rho=0}
  = \frac{\sigma}{2}\sum_{j=1}^{N-1}t_{Nj}^2.
\end{equation}

Substituting $\nu =1/2$ and using \eqref{dQ_drho} and \eqref{dS_dparams}, our third and final constrained maximization
equation is
\begin{align*}
...
%  &= -\frac{1}{2\rho}\sum_{i>j>0}\omega'_{ij}\left(1 + \frac{\rho{t_{ij}}}{(1 - e^{\rho{t_{ij}}}})\right)
  +\frac{\sigma}{2\rho^2}\sum_{j=1}^{N-1}\left(1 - e^{-\rho{t_{Nj}}} - \rho t_{Nj}e^{-\rho t_{Nj}}\right).
\end{align*}
