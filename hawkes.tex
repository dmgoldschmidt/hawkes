...
the EM algorithm.
Thus, if the event $T_i = t_i$ was generated by process ${\cal{P}}_j$ for some index $j < i$,
its density is
\begin{equation}\label{p_ij}
  p_{ij} :=  \lambda_{ij}e^{-(t_i-t_j)(1-\delta_{j0})\lambda_{ij}}\sum_{k=1}^i\frac{(\lambda_{ij}(t_i-t_j))^k}{k!}.
\end{equation}
by \eqref{Pr(obs|state)}, where $m = i-j-1$, $\delta_{j0}$ is the Kronecker delta, and
$\lambda_{ij} = \lambda_j(t_i)$ was defined above.

Unfortunately, however, the value of $j$ is not observed, i.e. we don't know
which process generated the $i^{th}$ event, so we can't use \eqref{Pr(obs|state)} directly. Instead,
