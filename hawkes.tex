...
case we denote the mark of event $i$ by $m(i)$ for $i > 0$, and we allow the exponential decay parameters for
child process
${\cal{P}}_j$ to depend on $m(j)$, the mark of the originating event at arrival time $t_j$.  Then for
...
$\lambda_{i0} := \lambda_0$ for all $i$, and $t_{ij} := t_i-t_j$ for $i > 0$..

\subsection{The Model}
We want to explain an observed arrival stream $\{(t_1,m(1),\dots,(t_N,m(N)\}$ as the output
of a Hawkes process.
...
Given the data $\{(t_1,m(t_i),\dots(t_N,m(t_N)\}$, we let $M$ denote the set of distinct marks $m(t_i)$
...
We next solve for $\lambda_0$, by recalling that $\lambda_{i0} = \lambda_0$, $\hat{k}_{i0} = \lambda_0t_{i0}$,
and that $q_{ij}$ does not depend on $\lambda_0$ for $j > 0$.
\begin{align*}
  0 = \frac{\partial\tilde{Q}}{\lambda_0} &= \frac{\partial}{\partial\lambda_0}\sum_{i=1}^N\omega'_{i0}\log\left(\omega_0\frac{\lambda_0^{\lambda_0t_{i0}}t_{i0}^{(\lambda_0t_{i0}-1)}}{\Gamma(\lambda_0t_{i0})}e^{-\lambda_0t_0} \right)\\
  &= \frac{\partial}{\partial\lambda_0}
  \sum_{i=1}^N\omega'_{i0}\left(\log(\omega_0)+\lambda_0t_{i0}\log(\lambda_0)+(\lambda_0t_{i0}-1)\log(t_{i0})
  -\log(\Gamma(\lambda_0t_{i0})) -\lambda_0t_{i0}\right)\\
  &= \sum_{i=1}^N\omega'_{i0}\left(t_{i0}(\log(\lambda_0+1) + t_{i0}\log(t_{i0})
  -\frac{\Gamma'(\lambda_0t_{i0})t_{i0}}{\Gamma(\lambda_0t_{i0})} -t_{i0}\right)\\
  &= \sum_{i=1}^N\omega'_{i0}t_{i0}(\log(\lambda_0t_{i0})  -\psi(\lambda_0t_{i0})),\\
\end{align*}
