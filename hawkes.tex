...
We first compute
$$
a_{ij} := \frac{\partial{f}}{\partial{x_j}}(\hat{X}_i),\quad\text{and}\quad b_i := y_i - f(X_i).
$$
...
$$
obtaining an overdetermined linear system $A\Delta{X} = b$, 
where
$$
A := [a_{ij}], \quad b := [b_i],\quad\text{and}\quad \Delta{X} :=  
...
overdetermined linear equations in $\Delta\sigma_m$ and $\Delta\rho_m$ which we use to reestimate $\sigma_m$ and
$\rho_m$.  We can choose to iterate by recomputing partials, or just proceed with another EM iteration.
