...
$t_j~(j > 0)$ by ${\cal{P}}_j$.  Suppose we observe arrival times $t_1,\dots,t_N$. Then for $j>0$,  ${\cal{P}}_j$ has rate
...
\end{cases}
\end{equation}
where $\mu$ and $\nu$ are parameters of the model.  We define $\lambda_{ij} := \lambda_j(t_i)$ for $0 <j < i$,
and $\lambda_{i0} := \lambda_0$ for all $i$.
...
of a Hawkes process by choosing the above parameters to maximize the likelihood of the observations using
the EM algorithm.
Thus, if $t_i$ is the arrival of the $k_j^{th}$ event generated by process ${\cal{P}}_j$ for some index $j < i$,
its density is
\begin{equation}\label{p_ij:0}
  p_{ij} :=  \frac{\lambda_{ij}^{k_j}(t_i-t_j)^{(k_j-1)}}{(k_j-1)!}e^{-\lambda_{ij}(t_i-t_j)}
\end{equation}
by \eqref{erlang}, where $\lambda_{ij} = \lambda_j(t_i)$ was defined above.
...
will replace $k_j$ by its expected value $\hat{k}_j$, which is easily seen to be given by
$$
\hat{k}_j = \int_{t_j}^{t_i}\lambda_j(t)dt = \frac{\mu}{\nu}(1-e^{-\nu(t_i-t_j)}).
$$
...
where 
\begin{equation}
  Pd(T_i = t) = \sum_{j = 0}^{i-1}\omega_j\frac{\lambda_{ij}^{\hat{k}_j}(t_i-t_j)^{(\hat{k}_j-1)}}{\Gamma(\hat{k}_j)}
    e^{-\lambda_{ij}(t_i-t_j)},
\end{equation}
where we have replaced $k_j$ by $\hat{k}_j$ and $(k_j-1)!$ by $\Gamma(\hat{k_j})$ (because $\hat{k}_j$ is not
in general an integer).
Our model parameters are
$$
\theta := \{\lambda_0, \mu,\nu,\omega_0,\dots,\omega_N\}.
$$
and the Q-function to be maximized (see \eqref{Qfunc:0}) becomes
$$
\tilde{Q}(\theta) = \sum_{i=1}^N\sum_{j=0}^N\omega_{ij}'\log(\omega_jp_{ij}),
$$
...
treating the density $\omega'_jp'_{ij}$ as a likelihood and normalizing by
the sum:
$$
\omega'_{ij} := \frac{\omega'_jp'_{ij}}{\sum_j\omega'_jp'_{ij}}.
$$
...
\begin{align*}
  D_{ij} := \frac{\partial{p_{ij}}}{\partial\lambda_{ij}} &= \frac{\partial}{\partial\lambda_{ij}}\left(\lambda_{ij}e^{-(t_i-t_j)(1-\delta_{j0})\lambda_{ij}}\sum_{k=1}^i\frac{(\lambda_{ij}(t_i-t_j))^k}{k!}\right)\\
  &= e^{-(t_i-t_j)(1-\delta_{j0})\lambda_{ij}}\sum_{k=1}^i\frac{(\lambda_{ij}(t_i-t_j))^k}{k!}\\
  &-\lambda_{ij}(t_i-t_j)(1-\delta_{j0})e^{-(t_i-t_j)(1-\delta_{j0})\lambda_{ij}}\sum_{k=1}^i\frac{(\lambda_{ij}(t_i-t_j))^k}{k!}\\
  &+\lambda_{ij}e^{-(t_i-t_j)(1-\delta_{j0})\lambda_{ij}}\sum_{k=1}^i\frac{\lambda_{ij}^{k-1}(t_i-t_j)^k}{(k-1)!}.
\end{align*}
...
At first glance, it appears that we might have a large number of states because the number of states $N+1$ is (one) more than the number of observations,  However, note first that $p_{ij} = 0$ for $j \ge i$, and second that
