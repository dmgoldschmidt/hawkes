...
\begin{equation}\label{Q1}
  \begin{split}
    \tilde{Q} &= \sum_{i=1}^N\sum_{j=0}^{i-1}\omega'_{ij}\log\left(\omega_j\frac{\lambda_{ij}^{\k_{ij}}t_{ij}^{(\k_{ij}-1)}}
          {\Gamma(\k_{ij})}e^{-\lambda_{ij}t_{ij}} \right)\\
    &= \sum_{i=1}^N\sum_{j=0}^{i-1}\omega'_{ij}\left(\log(\omega_j)+\k_{ij}\log(\lambda_{ij})+(\k_{ij}-1)\log(t_{ij})
...
we first apply \eqref{Q1} for all $i$ and $j>0$ to obtain 
\begin{equation}\label{dQ_dlambda,k}
   \begin{split}
     \frac{\partial\tilde{Q}}{\lambda_{ij}} &= \sum_{i=1}^N\omega'_{ij}\left(\k_{ij}\lambda_{ij}^{-1}-t_{ij}\right)\\
    &= \sum_{i=1}^N\omega'_{ij}t_{ij}(\log(\lambda_{ij}t_{ij})  -\psi(\lambda_{ij}t_{ij})),\quad\text{and}\\
    \frac{\partial\tilde{Q}}{\k_{ij}} &= \sum_{i=1}^N\omega'_{ij}(\log(\lambda_{ij}t_{ij})+\psi(\k_{ij})).
   \end{split}
\end{equation}

Now we can solve for $\sigma_m$ and $\rho_m$:
\begin{equation}\label{dQ_dsigma,rho}
%  \begin{split}
  0 = \frac{\partial\tilde{Q}}{\partial\sigma_m} = \sum_{i=1}^N\sum_{j\in J(m)}\frac{\partial\tilde{Q}}{\partial\lambda_{ij}}\frac{\partial\lambda_{ij}}{\partial\sigma_m} +
    \frac{\partial\tilde{Q}}{\partial\k_{ij}}\frac{\partial\k_{ij}}{\partial\sigma_m}
%  \end{split}
\end{equation}
