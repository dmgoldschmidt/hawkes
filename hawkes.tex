...
child process
${\cal{P}}_j$ to depend on $\mu(j)$, the mark of the originating event at arrival time $t_j$.  Then for
$j>0$, ${\cal{P}}_j$ has rate
...
of a Hawkes process.  All arrival times are given as elapsed times from some arbitrary reference time $t_0$, so
$t_{i0}$ is just $t_i$.
Thus, if $t_i$ is the arrival time of an event generated by process ${\cal{P}}_j$ for some index $j < i$, and
${\cal{P}}_j$ has generated $k_{ij}-1$ previous events prior to time $t_i$, then 
...
which process ${\cal{P}}_j$ generated the event $T_i = t_i$, nor do we know how many events were generated by ${\cal{P}}_j$
...
Next: check equations for \hat{k}_{ij} = \frac{1}{\rho_m}(\sigma_m - \lambda_{ij})
Then fix eqref{Q1}. It's now wrong.
Then fix eqref{lambda0}
To solve for the remaining parameters, we first expand \eqref{Q}:
\begin{equation}\label{Q1}
  \begin{split}
    \tilde{Q} &= \sum_{i=1}^N\sum_{j=0}^{i-1}\omega'_{ij}\log\left(\omega_j\frac{\lambda_{ij}^{\lambda_{ij}t_{ij}}t_{ij}^{(\lambda_{ij}t_{ij}-1)}}{\Gamma(\lambda_{ij}t_{ij})}e^{-\lambda_{ij}t_{ij}} \right)\\
    &= \sum_{i=1}^N\sum_{j=0}^{i-1}\omega'_{ij}\left(\log(\omega_j)+\lambda_{ij}t_{ij}\log(\lambda_{ij})+(\lambda_{ij}t_{ij}-1)\log(t_{ij})
  -\log(\Gamma(\lambda_{ij}t_{ij})) -\lambda_{ij}t_{ij}\right)\\
  \end{split}
\end{equation}

Then for all $i$ and $j$, we have 
\begin{equation}\label{dQ_dlambda_ij}
   \begin{split}
     \frac{\partial\tilde{Q}}{\lambda_{ij}} &= \sum_{i=1}^N\omega'_{ij}\left(t_{ij}(\log(\lambda_{ij}+1)+(t_{ij})\log(t_{ij})
    -\frac{\Gamma'(\lambda_{ij}t_{ij})}t_{ij}{\Gamma(\lambda_{ij}t_{ij})} - t_{ij}\right)\\
    &= \sum_{i=1}^N\omega'_{ij}t_{ij}(\log(\lambda_{ij}t_{ij})  -\psi(\lambda_{ij}t_{ij})),\\
   \end{split}
\end{equation}
   where $\psi(x) := \frac{\Gamma'(x)}{\Gamma(x)}$ is the digamma function.
   
We next solve for $\lambda_0$, first recalling that $\lambda_{i0} = \lambda_0$, $\hat{k}_{i0} = \lambda_0t_0$,
and that $q_{ij}$ does not depend on $\lambda_0$ for $j > 0$.  Then substituting these values into \eqref{dQ_dlambda_ij}, we
have:
\begin{equation}\label{lambda_0}
  0 = \frac{\partial\tilde{Q}}{\lambda_0} = \sum_{i=1}^N\omega'_{i0}t_0(\log(\lambda_0t_0)  -\psi(\lambda_0t_0)).
\end{equation}
  We can solve \eqref{lambda_0} for
...
$$
Solving for $\sigma_m$ and specializing \eqref{Q1} again, we have
...
\begin{equation}\label{D_ij}
  \begin{split}
    D_{ij} := \frac{\partial{q_{ij}}}{\partial\lambda_{ij}} &= \frac{\partial}{\partial\lambda_{ij}}\left(\omega_j\frac{\lambda_{ij}^{\hat{k}_{ij}}t_{ij}^{(\hat{k}_{ij}-1)}}{\Gamma(\hat{k}_{ij})}e^{-\lambda_{ij}t_{ij}} \right)\\
    &= \omega_j\frac{t_{ij}^{(\hat{k}_{ij}-1)}e^{-\lambda_{ij}t_{ij}}}{\Gamma(\hat{k}_{ij})}\left(\hat{k}_{ij}\lambda_{ij}^{\hat{k}_{ij}-1} - t_{ij}\right)\\
  \end{split}
\end{equation}
