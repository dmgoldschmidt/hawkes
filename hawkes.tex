...
Now let $\nu(t)$ be the ``counting function'' which is the total number of arrivals up to time $t$.
\begin{Cor}
...
\end{cases}
$$
So we enforce the constraints $\Sigma_l = 0$  using lagrange multipliers $\eta_l$ for $(1\le l\le N)$:
\begin{equation}
  \begin{split}
...
Recall that $\lambda_0 = \lambda_{i0}$ is a constant, and $\k_{i0} = \lambda'_0t_i$  for all $i$.  Moreover,
the expected number of arrivals due to $\P_0$ in the interval $[t_{i-1},t_i]$ is
$$
\k_{i0} - \k_{i-1,0} = i\lambda'_0 - (i-1)\omega'_{i-1,0}
$$
In particular, these inter-arrival times are independent i.i.d. samples from the exponential distribution with
rate $\lambda_0$, so we set
\begin{equation}\label{lambda_0}
  \lambda_0 = \frac{1}{N}\sum_{i=1}^Ni\omega'_{i0} - (i-1)\omega'_{i-1}.
\end{equation}
