...
of a Hawkes process by choosing the above parameters to maximize the likelihood of the observations.  Then if the event $T_i = t_i$ was generated by process ${\cal{P}}_j$ for some positive index $j < i$,
its density is
...
which process generated the event, so we can't use \eqref{Pr(obs|state)} directly. Instead,
we define a hidden state space $S := \{0,1,2,\dots,N\}$ and assume that $j\in{S}$ with probability $Pr(j) := \omega_j$.
Then we have the mixture density
\begin{equation}
  Pd(T_i = t) = \sum_{j = 0}^{i-1}\omega_jp_{ij},
\end{equation}
so that our model parameters are
$$
\theta := \{\lambda_0, \mu,\nu,\omega_0,\dots,\omega_N\}.
$$
and the function to be maximized (see \eqref{Qfunc:0}) becomes
$$
...
given that it was generated by process ${\cal{P}}_j$.  We compute this by
...
We can maximize $\tilde{Q}(\theta)$ in closed form by setting partial derivatives with respect to $\theta$ to zero and solving the resulting equations
for the posterior parameters.  We first solve for $\omega_j$, keeping in
mind the constraint
$$
...
$$
we conclude that $\rho = N$ and thus \eqref{omega_j:0} yields
